...
var CLog:Cls_LogLine=Cls_LogLine.new("ScnMnInputs");
...
func _ready():
	...
	V.V_ClsKeys.F_SetArray(m_TKeys);
...
func F_AddAcc():
	var M_LogVis:bool=false;CLog.Add("<F_AddAcc()>",M_LogVis);
	...
	CLog.Com("Accs:"+str(M_LstAcc.size()),false);
	# Por cada Accion P1
	for Mq in M_LstAcc.size():
		M_Acc=M_LstAcc[Mq];
		CLog.Com(str(Mq)+" Acc:"+str(M_Acc),false);
...
func EVT_on_button_up(SELF:Button):
	if (V_AccName2Scan==""):
		var M_LogVis:bool=false;CLog.Add("EVT_on_button_up("+SELF.name+")>",M_LogVis);
		
		var M_Acc:String=SELF.name.substr(4);#Texto del nombre del boton.
		CLog.Com("Acc:"+M_Acc,M_LogVis);
		V_AccName2Scan=M_Acc;
		V_ButtonSell=SELF;
		
		V_LbPressKey.visible=true;
		V_ScrlCont.visible=false;
		CLog.Com("CAD_FILE"+str(V.V_Keys.F_KeysGetArray()),M_LogVis);
		CLog.Del("...ok",M_LogVis);
	#END If
#END _on_button_up
...
func F_GetAccTxt(M_Acc:String)->String:
	var M_LogVis=false;CLog.Add("<F_GetAccTxt>",M_LogVis);
	var M_Res="";
	var M_Accs:Array=InputMap.get_actions();
	var M_Evt;
	
	CLog.Com("Acc:"+M_Acc+" Accs:"+str(M_Accs.size()),M_LogVis);
	for Mq in M_Accs.size():
		
		if(M_Accs[Mq]==M_Acc):
			M_Evt=InputMap.action_get_events(M_Acc);
			CLog.Com("Evts:"+str(M_Evt.size()),M_LogVis);
			
			if(M_Evt.size()>0):
				M_Evt=M_Evt[0];
				
				if(M_Evt is InputEventJoypadButton):
					CLog.Com("Tipe:JPB",M_LogVis);
...
func _input(event):
	...
	if (V_AccName2Scan != ""):
		...
		if event is InputEventKey:
			CLog.Add("<_input(KEY)>",M_LogVis);
			
			var M_Evts=InputMap.action_get_events(V_AccName2Scan);
			CLog.Com("ACC:"+V_AccName2Scan+" Evts:"+str(M_Evts.size()) ,M_LogVis);
			InputMap.action_erase_events(V_AccName2Scan);
			CLog.Com("Erase:"+str(M_Evts.size()) ,M_LogVis);
			...
			CLog.Com("Add Evt:"+str(M_Evts.size()) ,M_LogVis);
			V_ButtonSell.text=F_GetAccTxt(V_AccName2Scan);
			CLog.Com("Bt "+str(V_ButtonSell.name) ,M_LogVis);
			V_AccName2Scan = ""
			V_LbPressKey.visible=false;
			V_ScrlCont.visible=true;
			CLog.Del("<_input(KEY)...ok>",M_LogVis);
		#END IF Key
		elif event is InputEventJoypadButton:
			CLog.Add("<_input(JPB)>",M_LogVis);
			var M_EvtJoyBt:InputEventJoypadButton = event;
			var M_EvtNew:InputEventJoypadButton=InputEventJoypadButton.new();
			InputMap.action_erase_events(V_AccName2Scan);
			CLog.Com("Dell Accs:"+str(V_AccName2Scan) ,M_LogVis);
			...
			V_LbPressKey.visible=false;
			V_ScrlCont.visible=true;
			CLog.Del("<_input(JPB)...ok>",M_LogVis);
		#END IF JPB
		elif event is InputEventJoypadMotion:
			CLog.Add("<_input(JPM)>",M_LogVis);
			...
			if(M_EvtJoyMot.axis_value==-1 || M_EvtJoyMot.axis_value==+1):
				...
				V_ScrlCont.visible=true;
			#END Values Maximos
			CLog.Del("<_input(JPM)...Ok>",M_LogVis);
		#END IF JPM
		CLog.Add("<..OK>",false);
